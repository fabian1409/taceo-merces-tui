...
macro_rules! layout {
    (
        ...
        $(
            ...
            [
                $(
                    { name = $name:ident $(, width = $width:expr)? }
                ),*
            ]
        ),*
        $(,)?
    ) => {
        let __row_areas = ratatui::layout::Layout::vertical(vec![
            $(
                layout!(@get_height $( $height)? )
            ),*
        ]).split($area);
        let mut __rows = __row_areas.into_iter();
        $(
            let __col_areas = ratatui::layout::Layout::horizontal(vec![
                $(
                    layout!(@get_width $( $width )? )
                ),*
            ]).spacing(layout!(@get_spacing $( $spacing )? )).split(*__rows.next().unwrap());
            let mut __cols = __col_areas.into_iter();
            $(
                let $name = *__cols.next().unwrap();
            )+
        )*
    };

    (@get_height $height:expr) => { $height };
    (@get_height) => { Constraint::Fill(1) };

    (@get_width $width:expr) => { $width };
    (@get_width) => { Constraint::Fill(1) };

    (@get_spacing$spacing:expr) => { $spacing};
    (@get_spacing) => { 0 };
}
